...
def main(args):
    # Ensure module-level access to args for class methods (e.g., Chro.coordinate)
    # When LINKVIEW is imported and main() is called by another script, there is
    # no global 'args' defined unless we register it here.
    globals()['args'] = args
...
#==================creat svg=================
    ...
    for relation in relations:
        ...
        # 记录对齐块的包围盒用于比例尺位置避让
        xs = [vertex1[0], vertex2[0], vertex3[0], vertex4[0]]
        ys = [vertex1[1], vertex2[1], vertex3[1], vertex4[1]]
        align_bboxes.append((min(xs), min(ys), max(xs), max(ys)))
        #贝塞尔曲线风格
        particular_style = ''
    ...
    # 比例尺位置：默认右下角；如与对齐块重叠，尝试其他角
    def _scale_pos_candidates():
        right_x = args.svg_width*(1-args.svg_space/2)-L*scale
        left_x = args.svg_width*(args.svg_space/2)
        # 使用用户指定的比例或默认 0.9 作为底部；顶部固定为 0.1
        sy_ratio = getattr(args, 'scale_y_ratio', 0.9)
        try:
            sy_ratio = float(sy_ratio)
        except Exception:
            sy_ratio = 0.9
        sy_ratio = max(0.0, min(1.0, sy_ratio))
        bottom_y = args.svg_height * sy_ratio
        top_y = args.svg_height * 0.1
        return [
            (right_x, bottom_y),
            (left_x, bottom_y),
            (right_x, top_y),
    ...
    # 候选位置依次尝试，选择第一个不与任何对齐块包围盒相交的位置
    chosen_x, chosen_y = None, None
    for cx, cy in _scale_pos_candidates():
        sx1 = cx; sx2 = cx + L*scale
        sy1 = cy - 20; sy2 = cy + 5
        hit = False
        for bx1, by1, bx2, by2 in align_bboxes:
            if _overlaps(sx1, sy1, sx2, sy2, bx1, by1, bx2, by2):
                hit = True
                break
        if not hit:
            ...
            break
    if chosen_x is None:
        # 若所有候选都重叠，仍然选右下角但稍向下微调（不超过画布）
        default_x = args.svg_width*(1-args.svg_space/2)-L*scale
        default_y = args.svg_height * 0.95
        chosen_x, chosen_y = default_x, min(args.svg_height - 5, default_y)
    scale_x = chosen_x
    scale_y = chosen_y
    if(not args.no_scale):
        ...
        svg_content_scale.append(
            '<rect x="{}" y="{}" width="{}" height="{}" fill="none" opacity="0" class="scale-bbox"/>'.format(scale_x, scale_y-20, L*scale, 25)
        )
        svg_content_scale.append(
    ...
    if args.svg2png == 'inkscape':
        ...
        if _HAS_CAIROSVG:
            ...
            sys.stderr.write('warning: cairosvg not available; skip PNG conversion\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scale_y_ratio', default=0.9, type=float, help='比例尺纵向位置（相对 SVG 高度 0-1），默认 0.9')
    ...
    # 关键修复：把传入的形参注册到模块级，供类方法（如 Chro.coordinate）使用
    globals()['args'] = args
    main(args)
